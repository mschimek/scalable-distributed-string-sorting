...
string_set::string_set(std::vector<dsss::char_type>&& string_data)
    ...
    if constexpr (debug) {
        ...
        size_t avg_length = (strings_raw_data_.size() - strings_.size()) / (strings_.size() - 1);

        for (int32_t rank = 0; rank < env.size(); ++rank) {
            if (rank == env.rank()) {
                std::cout << rank << ": min " << min_length << ", max " << max_length << ", avg "
                          << avg_length << ", count " << strings_.size() - 1 << std::endl;
            }
            env.barrier();
        }
    }
...
string_set& string_set::operator=(string_set&& other) {
    if (this != &other) {
        auto const* old_ptr = other.strings_raw_data_.data();
        strings_raw_data_ = std::move(other.strings_raw_data_);
        strings_ = std::move(other.strings_);
        auto const* new_ptr = strings_raw_data_.data();
...
dsss::string string_set::operator[](const size_t idx) const { return strings_[idx]; }

std::vector<dsss::string>::const_iterator string_set::cbegin() { return strings_.cbegin(); }
...
std::vector<dsss::char_type>& string_set::data_container() { return strings_raw_data_; }
