...
public:
    ...
    UIntPair(Low const& l, High const& h) : low_(l), high_(h) {}

    //! copy constructor
    UIntPair(UIntPair const&) = default;
    //! move constructor
    UIntPair(UIntPair&&) = default;

    //! const from a simple 32-bit unsigned integer
    UIntPair(uint32_t const& a) // NOLINT
        : low_(a),
          high_(0) {}

    //! const from a simple 32-bit signed integer
    UIntPair(int32_t const& a) // NOLINT
        : low_(a),
          high_(0) {
        if (a >= 0)
            low_ = a;
    ...
    UIntPair(unsigned long long const& a) // NOLINT
        : low_((Low)(a & low_max())),
    ...
    UIntPair(unsigned long const& a) // NOLINT
        : UIntPair(static_cast<unsigned long long>(a)) {}

    //! construct from an 64-bit signed integer
    UIntPair(int64_t const& a) // NOLINT
        : UIntPair(static_cast<unsigned long long>(a)) {}

    //! copy assignment operator
    UIntPair& operator=(UIntPair const&) = default;
    ...
    UIntPair& operator+=(UIntPair const& b) {
    ...
    //! addition operator (uses 64-bit arithmetic)
    UIntPair operator+(UIntPair const& b) const {
        uint64_t add = low_ + uint64_t(b.low_);
        return UIntPair(
            (Low)(add & low_max()),
    ...
    UIntPair operator+(uint64_t const& b) const { return UIntPair(ull() + b); }
    ...
    bool operator==(UIntPair const& b) const { return (low_ == b.low_) && (high_ == b.high_); }

    //! inequality checking operator
    bool operator!=(UIntPair const& b) const { return (low_ != b.low_) || (high_ != b.high_); }

    //! less-than comparison operator
    bool operator<(UIntPair const& b) const {
        return (high_ < b.high_) || (high_ == b.high_ && low_ < b.low_);
    }

    //! less-than comparison operator
    bool operator<(uint64_t const& b) const { return ull() < b; }
    ...
    friend std::ostream& operator<<(std::ostream& os, UIntPair const& a) { return os << a.ull(); }
...
public:
    ...
    //! number of binary digits (bits) in UIntPair
    static int const digits = UIntPair::digits;
