...
environment::environment(environment const& other) = default;
...
void environment::finalize() { MPI_Finalize(); }

std::uint32_t environment::rank() const { return world_rank_; }

std::uint32_t environment::size() const { return world_size_; }

MPI_Comm environment::communicator() const { return communicator_; }

void environment::barrier() const { MPI_Barrier(communicator_); }
