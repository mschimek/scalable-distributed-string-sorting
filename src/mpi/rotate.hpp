...
void rotate_strings_right(
    ...
    // clang-format off
    if (skip_rank) {
        comm.recv(kmp::recv_buf(send_count), kmp::recv_count(1), kmp::source(pred), kmp::tag(tag));
    ...
    if (skip_rank) {
        comm.recv(kmp::recv_buf(dest), kmp::recv_count(recv_count), kmp::source(pred), kmp::tag(tag));
