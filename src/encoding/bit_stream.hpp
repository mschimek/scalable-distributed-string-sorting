...
public:
    explicit BitStreamWriter(BlockWriter& block_writer) : block_writer_(block_writer) {
        die_unless(block_writer.block_size() % sizeof(size_t) == 0);
    }

    //! non-copyable: delete copy-constructor
    BitStreamWriter(BitStreamWriter const&) = delete;
    //! non-copyable: delete assignment operator
    BitStreamWriter& operator=(BitStreamWriter const&) = delete;
    //! move-constructor: default
    BitStreamWriter(BitStreamWriter&&) = default;
    //! move-assignment operator: default
    BitStreamWriter& operator=(BitStreamWriter&&) = default;

    ~BitStreamWriter() { FlushBits(); }
     ...
     */
    void PutBits(size_t const& value, unsigned bits) {
        // check that only valid bits are set in value
        assert(bits == 64 || (value & (~size_t(0) << bits)) == 0);

        if (pos_ + bits > buffer_bits_) {
            // buffer overflown
            int length_first = buffer_bits_ - pos_, length_second = bits - length_first;
...
public:
    explicit BitStreamReader(BlockReader& block_reader) : block_reader_(block_reader) {}

    //! non-copyable: delete copy-constructor
    BitStreamReader(BitStreamReader const&) = delete;
    //! non-copyable: delete assignment operator
    BitStreamReader& operator=(BitStreamReader const&) = delete;
    //! move-constructor: default
    ...
    size_t GetBits(unsigned bits) {
        ...
        if (pos_ + bits > buffer_bits_) {
            ...
            int bits_first = buffer_bits_ - pos_ + 1, bits_second = bits - bits_first;
