...
static inline IntegerType AddTruncToType(IntegerType const& a, IntegerType const& b) {
...
public:
    ...
    static Range Invalid() {
        return Range(std::numeric_limits<size_t>::max(), std::numeric_limits<size_t>::min());
    }
    ...
    Range operator+(size_t const& shift) const { return Range(begin + shift, end + shift); }

    //! true if the Range contains x
    bool Contains(size_t x) const { return x >= begin && x < end; }
    ...
    Range Partition(size_t i, size_t parts) const {
        assert(i < parts);
        return Range(CalculateBeginOfPart(i, parts), CalculateBeginOfPart(i + 1, parts));
    }
 ...
 */
static inline size_t CalcOneFactorSize(size_t n) { return n % 2 == 0 ? n - 1 : n; }
