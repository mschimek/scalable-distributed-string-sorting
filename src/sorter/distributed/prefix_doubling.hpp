...
std::vector<size_t> computeResultsWithChecks(StringPtr local_string_ptr) {
    ...
    for (size_t i = startDepth; i < std::numeric_limits<size_t>::max();
        ...
        candidates =
            bloomFilter.filter(local_string_ptr, i, candidates, results);
        ...
        dss_schimek::mpi::execute_in_order([&]() {
            ...
            if (candidates != candidates_tracker) {
                size_t smallerSize =
                    std::min(candidates.size(), candidates_tracker.size());
                std::cout << "rank: " << env.rank() << std::endl;
                for (size_t i = 0; i < smallerSize; ++i) {
                    std::cout
                        << i << " candidate: " << candidates[i]
                        << " candidates_tracker: " << candidates_tracker[i]
                        << std::endl;
                    if (candidates[i] != candidates_tracker[i]) {
                        tracker.printDuplicateWitness(
                            candidates_tracker[i], env.rank());
                        std::abort();
                    }
                }
            ...
            if (results != results_tracker) {
                ...
                for (size_t i = 0; i < results.size(); ++i) {
                    std::cout << i << " results " << results[i]
                              << " results_tracker " << results_tracker[i]
                              << std::endl;
                    if (results[i] != results_tracker[i]) std::abort();
                }
            }
        });
...
std::vector<size_t> computeDistinguishingPrefixes(StringPtr local_string_ptr) {
    ...
    std::vector<size_t> candidates =
        bloomFilter.filter(local_string_ptr, startDepth, results);
    ...
    std::vector<StringIndexPEIndex> sort(
        ...
        for (auto str : sortedSet)
            permutation.emplace_back(
                sortedSet.getIndex(str), sortedSet.getPEIndex(str));
