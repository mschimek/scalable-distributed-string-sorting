...
getContiguousStrings(StringSet const& ss, size_t hintNumChars = 1000) {
    ...
    for (size_t i = 0; i < ss.size(); ++i) {
        ...
        std::copy_n(
            ss.get_chars(str, 0),
            ss.get_length(str) + 1,
            std::back_inserter(contiguousStrings)
        );
    }
...
static inline size_t string_length(CharType const* str);
...
template <typename CharType>
static inline void print(CharType const* raw_strings, size_t size) {
...
template <typename CharType>
static inline void print(std::vector<CharType> const& raw_strings) {
...
static inline std::string toBinary(Type v, int const width = (1 << sizeof(Type))) {
    char binstr[width + 1];
    binstr[width] = 0;
...
static inline int scmp(const string _s1, const string _s2, size_t& lcp) {
...
static inline int scmp(unsigned char const* _s1, unsigned char const* _s2) {
    unsigned char const* s1 = _s1;
    unsigned char const* s2 = _s2;
...
template <typename CharT>
static inline CharT get_char(string ptr, size_t depth) __attribute__((always_inline));
...
template <typename T>
static inline int count_high_zero_bits(T const& t) __attribute__((always_inline));

template <>
inline int count_high_zero_bits<uint32_t>(uint32_t const& t) {
    if (t == 0)
        return sizeof(t) * 8;
...
template <>
inline int count_high_zero_bits<uint64_t>(uint64_t const& t) {
    if (t == 0)
        return sizeof(t) * 8;
...
template <>
inline int count_high_zero_bits<uint128_t>(uint128_t const& t) {
    if (t == 0)
        return sizeof(t) * 8;
    uint64_t hi = (t >> 64);
...
template <typename T>
static inline int count_low_zero_bits(T const& t) __attribute__((always_inline));

template <>
inline int count_low_zero_bits<uint32_t>(uint32_t const& t) {
    if (t == 0)
        return sizeof(t) * 8;
...
template <>
inline int count_low_zero_bits<uint64_t>(uint64_t const& t) {
    if (t == 0)
        return sizeof(t) * 8;
...
calculateRanges(std::pair<size_t, size_t>* ranges, unsigned numberOfSplits, size_t lengthToSplit) {
    ...
    ranges[numberOfSplits - 1] =
        std::make_pair((numberOfSplits - 1) * split, lengthToSplit - (numberOfSplits - 1) * split);
}
...
struct TreeCalculations {
    ...
    static inline unsigned int level_to_preorder(unsigned int id) {
        ...
        static int const bitmask = numnodes;
