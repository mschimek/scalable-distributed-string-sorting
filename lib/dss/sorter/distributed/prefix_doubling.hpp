...
StringLcpContainer<StringSet> receive_strings(
    ...
    std::vector<size_t> req_sizes(comm.size());
    ...
    auto const recv_req_sizes = comm.alltoall(kamping::send_buf(req_sizes));
    auto const recv_requests =
        comm.alltoallv_dispatch(requests, req_sizes, recv_req_sizes, alltoallv_params);
    ...
    auto recv_buf_char = comm.alltoallv_dispatch(raw_strs, raw_str_sizes, alltoallv_params);
...
StringLcpContainer<StringSet> apply_permutation(
    ...
    auto output_container =
        _internal::receive_strings(ss, permutation, comm, alltoallv_params);
